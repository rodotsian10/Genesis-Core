...
## 📌 특이사항 03: 유전 폭주와 초우점 거인종의 자원 고갈 재앙 (페이즈 6)

> [!WARNING]
> **현상 요약:**
> 페이즈 6 밸런싱 도중, 10%의 확률로 발생하는 돌연변이(대돌연변이) 효과가 수십 세대에 걸쳐 누적되면서 **크기 13.0배, 속도 74.0배, 대사랑 0.01**이라는 생태 파괴적인 초우량 사기 개체가 출현했습니다. 이들이 번식을 완벽히 독점하여 맵의 모든 식량을 고갈(`0/2000`)시키고 약소 개체를 멸종시킨 유전학적 재앙입니다.

### 1. 현상 캡처 및 기록 자료
![초우점 거인종 발생 및 식량 전멸 현장](file:///e:/godsimulate/docs/특이사항/giant_catastrophe.png)

### 2. 세부 관찰 및 문제점 분석

* **극단적인 유전 폭주:**
  - **ID 52698 개체**는 수명은 184초에 불과하지만, **크기(Size) 13.04**로 일반 개체의 10배가 넘는 면적을 차지합니다.
  - **속도(Speed)는 74.27**로 번개처럼 맵 전역을 종횡무진하며, **대사랑(Meta)이 0.01**로 거의 기력을 소모하지 않습니다.
  - 먹는 족족 100% 에너지로 치환되는 초효율 생명체이기에 굶어 죽을 리가 없으며, 압도적인 기동성으로 모든 밥을 독점했습니다.
* **약소 종족의 멸종과 자원 잠김:**
  - 화면에 거대하고 알록달록한 블록들이 가득 들어찼는데, 이는 모두 사기 유전자를 물려받은 초우량 거인종의 후손들입니다.
  - 거인종들이 맵 전역의 밥을 순간적으로 전부 휩쓸어가 식량이 상시 **`0/2000`**으로 유지되는 대기근이 발생했습니다.
  - 이에 따라 평범한 육지/바다 개체들은 먹이를 한 톨도 구하지 못하고 **대규모 아사(때죽음)**로 이어졌습니다.
* **대응 방안:**
  - 유전자의 무한 진화 폭주를 막기 위해 **속도 및 크기 유전자의 강제 한계선(속도 최대 6.0 등 Hard Clamp)**을 도입할 예정입니다.
  - 또한 속도가 빨라질수록 수명이 극도로 짧아지는 **"Live Fast, Die Young"** 생물학적 패널티 메커니즘을 추가하여 유전 밸런스를 조율할 계획입니다.
